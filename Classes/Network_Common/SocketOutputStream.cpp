...
//	Assert( m_pSocket != NULL );
    ...
    if ( 0 == Msg_Manager::instance()->is_send_msg_empty() ) 
        return 0;
	
    md.reset();
    Msg_Manager::instance()->pop_send_msg_list(md);
    nSent = m_pSocket->send( md.get_buff(), md.get_size() , flag );
    //CCLOG("windows send packet result %d", nSent);
    if (nSent==SOCKET_ERROR_WOULDBLOCK) return 0 ; 
    if (nSent==SOCKET_ERROR) return SOCKET_ERROR-2 ;
    if (nSent==0) return 0;
    nFlushed += nSent;
    //CCLOG("windows send packet succed %d", nSent);    
