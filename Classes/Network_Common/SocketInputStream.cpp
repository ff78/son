...
BOOL SocketInputStream::Peek( UINT len ) 
{
__ENTER_FUNCTION_FOXNET
...
//	Assert( buf!=NULL ) ;	
	...
	const CHAR* pHead = &m_Buffer[m_Head];

	int _pack_len = *((int*)(pHead));

	if ( (UINT)_pack_len > Length() )
	{
		return FALSE;
	}
	...
	Msg_Data _msg;
	_msg.reset();
	_msg.set_buff(pHead, _pack_len);

	Msg_Manager::instance()->push_msg_to_list( _msg );
