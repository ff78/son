...
//USING_NS_CC_EXT;
using namespace ui;
enum Residual_Page_Back_Child
{
    RPBC_NUM = 123456789,
    RPBC_SHOW = 987654321,
};

UI_Residual_Page_Layer::UI_Residual_Page_Layer(void) :
...
bool UI_Residual_Page_Layer::init()
{
	bool bRet = false;
	do 
	{
		...
		rootWidget = cocostudio::GUIReader::getInstance()->widgetFromJsonFile("ui/Residual_Page/Residual_Page.ExportJson");
		...
		btnClose->addTouchEventListener(this, toucheventselector(UI_Residual_Page_Layer::buttonCloseCallback));
...
void UI_Residual_Page_Layer::initComponentOfUI()
{
	...
	btnBook = dynamic_cast<cocos2d::ui::Button*>(Helper::seekWidgetByName(rootWidget,"btn_book"));
	btnBook->addTouchEventListener(this, toucheventselector(UI_Residual_Page_Layer::buttonBookLayerCallback));

	btnCombine = dynamic_cast<cocos2d::ui::Button*>(Helper::seekWidgetByName(rootWidget,"btn_combine"));
	btnCombine->addTouchEventListener(this, toucheventselector(UI_Residual_Page_Layer::buttonCombineCallback));

	btnQualityUp = dynamic_cast<cocos2d::ui::Button*>(Helper::seekWidgetByName(rootWidget,"btn_quality_up"));
	btnQualityUp->addTouchEventListener(this, toucheventselector(UI_Residual_Page_Layer::buttonQualityUpCallback));
	...
	m_pLabTipsFont = dynamic_cast<cocos2d::ui::Text*>(Helper::seekWidgetByName(m_pImgTipsShow,"cocos2d::ui::Text"));
	const char* tipsFontInfo = DICTIONARY_CONFIG_MGR::instance()->get_string_by_id(RESIDUAL_PAGE_CURRENT_NO);
    ...
    for (int i = RESIDUAL_PAGE_TYPE_COUNT-1;i >= 0;--i)
    {
        //创建背景
      ...
      m_fragmentBack[i]->setSize(Size(500,115));
       ...
       m_fragmentBack[i]->addTouchEventListener(this, toucheventselector(UI_Residual_Page_Layer::selectCurMaterial));
      m_fragmentBack[i]->setTag(0);
		...
		m_iconTexture[i]->addTouchEventListener(this, toucheventselector(UI_Residual_Page_Layer::residualPageFindWay));
...
//        m_labIconName[i]->setFontSize(25);
      ...
      m_labIconNum[i]->setTag(RPBC_NUM);
//        m_labIconNum[i]->setFontSize(25);
       ...
       m_btnCombine[i]->addTouchEventListener(this, toucheventselector(UI_Residual_Page_Layer::compoundMaterial));

        m_labBtnShow[i] = cocos2d::ui::Text::create();
        m_btnCombine[i]->addChild(m_labBtnShow[i]);
      m_labBtnShow[i]->setTag(RPBC_SHOW);
//        m_labBtnShow[i]->setFontSize(25);
    }
}

void UI_Residual_Page_Layer::buttonCloseCallback(Ref* pSender,Widget::TouchEventType type)
{
switch (type)
{
...
void UI_Residual_Page_Layer::update(int change)
{	
	buttonOpenOrNot();
    if (change == 1)
    {
...
void UI_Residual_Page_Layer::setVisible( bool visible )
{
...
void UI_Residual_Page_Layer::showGuide()
{
	...
	if( GUIDE_XF_PINZHUANG_3 == player->get_guide_id() )
	{
		if(m_btnCombine[3])
		{
			Joiner_Guidance::receiveCurPosWidget(Vec2(
				m_btnCombine[3]->getWorldPosition().x + 45,
				m_btnCombine[3]->getWorldPosition().y + 50) );
		}
	}
}

void UI_Residual_Page_Layer::change_refresh()
{
    reset_ui();
    Game_Data::Item_List book_fragment_list;
    int fragmentNum = ITEM_MGR::instance()->get_all_book_fragment(book_fragment_list);
	if(fragmentNum <= 0)
	{
    ...
    Game_Data::Item_List_Iter iter = book_fragment_list.begin();
    for (;iter != book_fragment_list.end() && index>=0 ;++iter)
    {
        item = *iter;
        if (!item)
        {
	...
	m_iconTexture[index]->setTag(item->config->base_id);
      ...
      m_btnCombine[index]->setTag(item->config->base_id);
        ...
        string = DICTIONARY_CONFIG_MGR::instance()->get_string_by_id(ASSEMBLE_COUNT);
...
void UI_Residual_Page_Layer::no_change_refresh()
{   
    ...
    for (;iter != m_list_fragment.end();++iter)
    {
        ...
        item = ITEM_MGR::instance()->get_book_fragment(fragmentBack->getTag());
        if (!item)
        {
            continue;
        }
        ...
        int need_count = reinterpret_cast<Game_Data::Item_Fragment_Config*>(item->config)->need_count;
        labIconNum = dynamic_cast<cocos2d::ui::Text*>(fragmentBack->getChildByTag(RPBC_NUM));
        if (labIconNum)
        {
            labIconNum->setString(CCString::createWithFormat("(%d/%d)",item->count,need_count)->getCString());
        }

        labBtnShow = dynamic_cast<cocos2d::ui::Text*>(fragmentBack->getChildByTag(RPBC_SHOW));
        string = DICTIONARY_CONFIG_MGR::instance()->get_string_by_id(ASSEMBLE_COUNT);
        if (labBtnShow)
        {
...
void UI_Residual_Page_Layer::reset_ui()
{
    m_pImgEffect->setVisible(false);
    cocos2d::ui::ImageView* fragmentBack = NULL;
...
void UI_Residual_Page_Layer::materialShow(int item_base_id)
{
...
void UI_Residual_Page_Layer::selectCurMaterial( Ref* pSender ,Widget::TouchEventType type)
{
...
void UI_Residual_Page_Layer::compoundMaterial( Ref* pSender,Widget::TouchEventType type )
{
...
switch (type)
{
    ...
    if (curImgBack)
    {
        ...
        ITEM_LOGIC::instance()->compound_book_fragment(curImgBack->getTag(),1);
...
void UI_Residual_Page_Layer::buttonBookLayerCallback( Ref* pSender ,Widget::TouchEventType type)
{
switch (type)
{
    case cocos2d::ui::Widget::TouchEventType::ENDED:
...
void UI_Residual_Page_Layer::buttonCombineCallback( Ref* pSender ,Widget::TouchEventType type)
{
switch (type)
{
    case cocos2d::ui::Widget::TouchEventType::ENDED:
...
void UI_Residual_Page_Layer::buttonQualityUpCallback(Ref* pSender,Widget::TouchEventType type)
{
switch (type)
{
    case cocos2d::ui::Widget::TouchEventType::ENDED:
...
void UI_Residual_Page_Layer::residualPageFindWay( Ref* pSender ,Widget::TouchEventType type)
{
...
void UI_Residual_Page_Layer::buttonOpenOrNot()
{
	//btnCombine->setVisible(UI_MainMenu_Layer::get_instance()->arr[Game_Data::ICON_XINFA_COMBINE_BTN]);
	//btnCombine->setTouchEnabled(UI_MainMenu_Layer::get_instance()->arr[Game_Data::ICON_XINFA_COMBINE_BTN]);

	//btnQualityUp->setVisible(UI_MainMenu_Layer::get_instance()->arr[Game_Data::ICON_XINFA_QUA_BTN]);
	//btnQualityUp->setTouchEnabled(UI_MainMenu_Layer::get_instance()->arr[Game_Data::ICON_XINFA_QUA_BTN]);
}
