...
#define ACCOUNT_FILE "player_account.json"
#define PLAYER_FILE "player.json"
#define INSTANCE_RESULT_FILE "instance_result.json"
#define STAGE_RESULT_FILE "stage_result.json"
#define ENDLESS_TOWER_RESULT_FILE "endless_result.json"
...
//    }
    ...
    temp.set_keyword(account.c_str());
    on_player_login(temp);
    return true;
}
...
//    }
   ...
   Account_Data_Mgr* ptr = Account_Data_Mgr::instance();
    ...
    int role_count = 0;
//    body_ms >> role_count;
    std::string account = gm_interface.get_keyword();
    auto keyValue = Value(account);
    rapidjson::Value root;
    rapidjson::Document doc;
    
//    std::string path3 = FileUtils::getInstance()->getWritablePath();
//    path3 += ACCOUNT_FILE;
//    remove(path3.c_str());
//    std::string path4 = FileUtils::getInstance()->getWritablePath();
//    path4 += PLAYER_FILE;
//    remove(path4.c_str());
//    std::string path = FileUtils::getInstance()->getWritablePath();
//    path += STAGE_RESULT_FILE;
//    remove(path.c_str());
//    std::string path1 = FileUtils::getInstance()->getWritablePath();
//    path1 += ENDLESS_TOWER_RESULT_FILE;
//    remove(path1.c_str());
//    std::string path2 = FileUtils::getInstance()->getWritablePath();
//    path2 += INSTANCE_RESULT_FILE;
//    remove(path2.c_str());
    
    bool found = GameReader::isExistInFile(ACCOUNT_FILE, keyValue, doc, root, "account");
    role_count = (found?1:0);
//    std::map<std::string, cocos2d::Value> values;
//    values["prim-key"] = account;
//    auto keyValue = Value(account);
//    GameReader::saveToFile(ACCOUNT_FILE, values, keyValue);
...
//			   //CCLog("--------------7777777777777777777777777777777777777777777777777777777777--------------");
           ...
           pADMgr->m_RoleData[i].m_nRoleID = DICTOOL->getIntValue_json(root, "roleId");
           pADMgr->m_RoleData[i].m_nBaseID = DICTOOL->getIntValue_json(root, "baseId");
           pADMgr->m_RoleData[i].m_nLevel = DICTOOL->getIntValue_json(root, "level");
           pADMgr->m_RoleData[i].m_nSex = DICTOOL->getIntValue_json(root, "sex");
           pADMgr->m_RoleData[i].m_strName = DICTOOL->getStringValue_json(root, "roleName");
...
bool Account_Msg_Proc::send_create_role( Game_Logic::Game_Interface& gm_interface )
{
    ...
    Account_Data_Mgr::instance()->setJob(job);
	//ƒø«∞÷ªø™∑≈3 Œ‰∂∑º“÷∞“µ
	//job = 3;
...
//    CNetManager::GetMe()->send_msg( body_ms );
    ...
    Game_Logic::Game_Interface temp;
    temp.set_keyword(role_name);
    on_player_create_role(temp);
    return 0;
}
...
//	{
	    int role_id = 0;
		int role_base_id = 1001;
		int role_level = 1;
        int role_sex = 0;
		std::string role_name = gm_interface.get_keyword();
//		body_ms >> role_id;
//		body_ms >> role_base_id;
//		body_ms >> role_level;
//        body_ms >> role_sex;
//		body_ms >> role_name;
    int role_size = GameReader::getFileSize(ACCOUNT_FILE);
    role_id = role_size + 1;
        ...
        std::map<std::string, cocos2d::Value> values;
        values["roleId"] = role_id;
        values["baseId"] = role_base_id;
        values["level"] = role_level;
        values["sex"] = role_sex;
        values["roleName"] = role_name.c_str();
        values["account"] = Account_Data_Mgr::instance()->get_self_account();
        auto keyValue = Value(Account_Data_Mgr::instance()->get_self_account());
    ACCOUNT_DOC_INFO a_info;
    a_info.roleId = role_id;
    a_info.baseId = role_base_id;
    a_info.level = role_level;
    a_info.sex = role_sex;
    a_info.roleName = role_name.c_str();
    a_info.account = Account_Data_Mgr::instance()->get_self_account();
    
        GameReader::saveToFile(ACCOUNT_FILE, a_info, keyValue, "account");
...
bool Account_Msg_Proc::on_save_player(Game_Logic::Game_Interface& gm_interface)
{
    ...
    std::map<std::string, cocos2d::Value> values;
    values["base_id"] = player->get_static_character_id();
    values["sex"] = player->get_sex();
    values["level"] = player->get_character_level();
    values["gold"] = player->get_gold();
    values["quality"] = player->get_character_quality();
    values["exp"] = player->get_exp();
    values["energy"] = player->get_energy();
    values["job"] = player->get_job();
    values["cur_god_id"] = player->get_cur_god_id();
    values["cur_pet_id"] = player->get_cur_pet_id();
    int list_size = (int)player->get_pets().size();
    values["list_size"] = list_size;
    for (int i = 0; i<list_size; i++) {
        if (i>=3) {
            break;
        }
        std::string pet("pets_id_");
        pet += StringUtils::toString(i);
        values[pet.c_str()] = player->get_pets()[i];
    }
    values["account"] = Account_Data_Mgr::instance()->get_self_account();
    auto keyValue = Value(Account_Data_Mgr::instance()->get_self_account());
    GameReader::saveToFile(PLAYER_FILE, values, keyValue, "account");
    

    ACCOUNT_DOC_INFO a_info;
    a_info.roleId = player_id;
    a_info.baseId = player->get_static_character_id();
    a_info.level = player->get_character_level();
    a_info.sex = player->get_sex();
    a_info.roleName = player->get_character_name();
    a_info.account = Account_Data_Mgr::instance()->get_self_account();
...
//    body_ms.set_rd_ptr(sizeof(int));
	...
	int camp_tag = 0;
	int main_city_scene_id = 1001;
    int job = Account_Data_Mgr::instance()->getJob();

	int faction_job = 0;
	int contribute_num = 0;
	int curr_digging_num = 0;
	int curr_worship_num = 0;
	int curr_boss_num = 0;
	int faction_id = 0;
    ...
    std::string name;
    name = gm_interface.get_keyword();
    std::string role_name = Account_Data_Mgr::instance()->get_self_account();
    
    auto keyValue = Value(role_name);
    rapidjson::Value root;
    rapidjson::Document doc;
    if(GameReader::isExistInFile(PLAYER_FILE, keyValue, doc, root, "account"))
    {
        base_id = DICTOOL->getIntValue_json(root, "base_id");
        sex = DICTOOL->getIntValue_json(root, "sex");
        level = DICTOOL->getIntValue_json(root, "level");
        gold = DICTOOL->getIntValue_json(root, "gold");
        quality = DICTOOL->getIntValue_json(root, "quality");
        exp = DICTOOL->getIntValue_json(root, "exp");
        energy = DICTOOL->getIntValue_json(root, "energy");
        job = DICTOOL->getIntValue_json(root, "job");
        cur_god_id = DICTOOL->getIntValue_json(root, "cur_god_id");
        cur_pet_id = DICTOOL->getIntValue_json(root, "cur_pet_id");
        list_size = DICTOOL->getIntValue_json(root, "list_size");
        for (int i = 0; i<list_size; i++) {
            if (i>=3) {
                break;
            }
            std::string pet("pets_id_");
            pet += StringUtils::toString(i);
            pets_id[i] = DICTOOL->getIntValue_json(root, pet.c_str());
        }
    }else{
        std::map<std::string, cocos2d::Value> values;
        ...
        values["job"] = job;
        values["cur_god_id"] = cur_god_id;
        values["cur_pet_id"] = cur_pet_id;
        values["list_size"] = list_size;
        for (int i = 0; i<list_size; i++) {
            if (i>=3) {
                break;
